...
(module+ test
  ...
  (define (property:board-correct-size)
    (check-equal? (length (territory-build)) GRID "board incorrect-size"))

  (define (property:no-pass-on-first-move)
    (define (move-action? m)
      (equal? (move-action m) '()))
  ...
  ;; legal?
  (check-true
   (and (attackable? (list (territory 0 0 2 9 0) (territory 3 1 1 9 0)) 0 (territory 0 0 2 9 0) 3)
        #t))
  (check-false
  ...
  (check game-tree=? (game-tree (list (territory 0 0 2 'x 'y) (territory 1 1 1 'a 'b)) 0 0) gt3)
  ...
  (define (property:attackable?-does-not-need-neighbores-check)
    (define (check-attackable? gt)
      (for/and ([move (game-moves gt)]
        ...
        (and (member (second action) (neighbors (first action))) (check-attackable? gt))))
  ...
  ;; ---------------------------------------------------------------------------------------------------
  (define board20 (list (territory 0 0 1 9 2) (territory 1 0 1 9 0) (territory 2 2 1 9 0)))
  (define board21
  ...
  (define tree0
    (game-tree
     (list (territory 0 1 3 0 0) (territory 1 0 2 0 0) (territory 2 0 2 0 0) (territory 3 0 2 0 0))
     1
     15))
  ...
  (define board31
    (list territory1 (territory 2 0 3 150 262.5) (territory 1 1 2 345 150) (territory 0 0 2 215 150)))

  (define world1 (dice-world #f board31 (game board31 1 (delay '()))))
  ...
  (check-equal? (interact-with-board (dice-world 3 '() '()) "d") (dice-world #f '() '()))

  (check-equal?
   (game-board (the-ai-plays tree0))
   (list (territory 3 1 3 0 0) (territory 2 0 2 0 0) (territory 1 0 2 0 0) (territory 0 1 2 0 0)))

  (check-equal? (game-player (the-ai-plays tree0)) 0)

  (check-equal?
   (game-board (move-gt (first (game-moves tree0))))
   (list (territory 0 1 1 0 0) (territory 1 0 2 0 0) (territory 2 0 2 0 0) (territory 3 1 2 0 0)))

  (check-equal? (game-player (move-gt (first (game-moves tree0)))) 1)

  (check-equal? (rate-position tree0 AI-DEPTH) 1/2)
  (check-equal? (rate-position (move-gt (first (game-moves tree0))) AI-DEPTH) 1/2)
