...
(module maze racket
  ...
  (define MAZE-STATE-WIDTH 10)
  (define MAZE-STATE-HEIGHT 10)
  (define policy-prob (make-parameter #f))
  (define local-reward (make-parameter #f))

  (struct maze-state (x y)
    #:transparent
    ...
    [(define (possible-action state)
     ...
     (define (take-action state action)
       ...
       (case action
         ['left (maze-state (sub1 x) y)]
         ['right (maze-state (add1 x) y)]
         ['up (maze-state x (sub1 y))]
         ['down (maze-state x (add1 y))]
    ...
    (define (update-path path reward)
      (let loop ([reverse-path (reverse path)]
        ...
        (unless (empty? reverse-path)
          ...
          (loop (cdr reverse-path) (+ (if edge (mcts-edge-reward edge) 0) (* gamma reward))))))
      ...
      (define (simulate state)
        (cond
          ...
          [else
           ...
           (+ (* gamma (simulate (take-action state action))) (process-reward state action))]))
...
(require (submod "." maze))
(require (submod "." mcts-generics))
(define episode-num 10)

(define param-range (map (lambda (a) a) (inclusive-range 1 5 1)))
param-range
