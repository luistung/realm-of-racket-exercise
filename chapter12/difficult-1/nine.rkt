...
(struct nine-state (board player)
  ...
  [(define (player mcts-state)
   ...
   (define (process-reward mcts-state action)
     (define terminal-player (nine-board-terminal? (nine-state-board mcts-state)))
     (define player (nine-state-player mcts-state))
     (cond
...
(let loop ([state state]
           [step 0])
  (cond
    [(or (terminal? state) (>= step 100)) (nine-board-print (nine-state-board state)) step]
    [else
     (define action (mcts-search state #:simulate-num 10 #:per-simulate-num 1 #:gamma 0.9))
