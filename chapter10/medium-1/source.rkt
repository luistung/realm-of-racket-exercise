...
(define (execute board player src dst dice)
  ...
  (cond [(> (random-dice dice) (random-dice dst-dice))
         (set! src-territory (territory-set-dice src-territory 1))
         (define s (territory-set-dice dst-territory (- dice 1)))
         (set! dst-territory (territory-set-player s player))]
        [else (set! src-territory (territory-set-dice src-territory 1))])
  (for/list ([t board])
    (define idx (territory-index t))
    (cond [(= idx src) src-territory]
