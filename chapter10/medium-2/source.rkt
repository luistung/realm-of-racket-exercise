...
(define (dice-world-board w)
  (game-board (dice-world-gt w)))
...
(define PROB-X-OFFSET 120)
(define PROB-Y-OFFSET 80)
...
(define (draw-dice-world w)
  ...
  (add-win-prob w (add-player-info (game-player (dice-world-gt w)) (add-board-to-scene w (ISCENE)))))
...
(define (execute board player src dst)
  ...
  (cond
    ...
    [else
     ...
     (set! src-territory (territory-set-dice src-territory 1))])
  (log-debug "src-dice ~s, dst-dice ~s\n"
             (territory-dice src-territory)
             (territory-dice dst-territory))
  (for/list ([t board])
...
(define dice-num-sum-count
  (for/hasheqv ([i (range 1 (add1 DICE#))])
    (values i
            ((lambda (h)
               (define sum (apply + (hash-values h)))
               (hash-map/copy h (lambda (k v) (values k (/ v sum)))))
             ((lambda (n)
                (define sum-count (make-hasheqv))
                (let loop ([left n]
                           [result '()])
                  (cond
                    [(zero? left)
                     (define sum (apply + result))
                     (hash-set! sum-count sum (add1 (hash-ref sum-count sum 0)))]
                    [else (for-each (lambda (i) (loop (sub1 left) (cons i result))) (range 1 7))]))
                sum-count)
              i)))))

(define (calc-prob a b)
  (define a-h (hash-ref dice-num-sum-count a))
  (define b-h (hash-ref dice-num-sum-count b))
  (exact->inexact
   (for*/sum ([i (hash-keys a-h)] [j (hash-keys b-h)])
            (if (> i j) (* (hash-ref a-h i) (hash-ref b-h j)) 0))))

(define (add-win-prob w s)

  (define player (game-player (dice-world-gt w)))
  (define board (dice-world-board w))
  (define dst-t (first board))
  (define p-focus (territory-player dst-t))

  (define src (dice-world-src w))

  (cond
    [src
     (define src-t (findf (lambda (t) (= (territory-index t) src)) board))
     (cond
       [(attackable? board player src-t (territory-index dst-t))
        (define prob (calc-prob (territory-dice src-t) (territory-dice dst-t)))
        (define str (format "probability: ~a%" (~r #:precision '(= 2) (* prob 100))))
        (define txt (text str TEXT-SIZE TEXT-COLOR))
        (place-image txt (- WIDTH PROB-X-OFFSET) PROB-Y-OFFSET s)]
       [else s])]
    [else s]))
...
(define (pass w)
  ...
  (cond
    ...
    [else
     ...
     (cond
       [(or (empty? (game-moves nextgt)) (not (= (game-player nextgt) AI))) (dice-world #f nextgt)]
       [else
