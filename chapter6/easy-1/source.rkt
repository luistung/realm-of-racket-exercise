...
(define (start-snake)
  (big-bang (pit (snake "right" (list (posn 1 1)))
                 (for/list ([_ (in-range INIT-GOO-NUM)]) (fresh-goo)))
...
(define (next-pit w)
  ...
  (if goo-to-eat
      (begin (set! eaten-num (add1 eaten-num)) (pit (grow snake) (age-goo (eat goos goo-to-eat))))
...
(define (render-end w)
  (overlay (text (format "Game over\nNumber of Pieces of goo: ~s" eaten-num) ENDGAME-TEXT-SIZE "black")
           (render-pit w)))
