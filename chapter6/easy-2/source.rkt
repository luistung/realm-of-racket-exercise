...
(define (eat goos goo-to-eat)
  (define new-goos (remove goo-to-eat goos))
  (define max-goos-num (max 0 (- MAX-GOO (length new-goos))))
  (define min-goos-num (if (empty? new-goos) 1 0))
  (for/foldr ([g new-goos])
             ([i (in-range (random min-goos-num (add1 max-goos-num)))])
             (cons (fresh-goo) g)))
...
(define snake0
  (snake "right" (list (posn 1 1)))) ;; BUG? << -- moving this define into the test module blows up
