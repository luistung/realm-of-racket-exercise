...
(struct pit (snake goos obstacles) #:transparent)
...
(define (next-pit w)
  ...
  (if goo-to-eat
      (pit (grow snake) (age-goo (eat goos goo-to-eat)) obstacles)
      (pit (slither snake) (age-goo goos) obstacles)))
...
(define (render-pit w)
  (snake+scene (pit-snake w)
               (obstacle+scene (pit-obstacles w) (goo-list+scene (pit-goos w) MT-SCENE))))
...
(define (world-change-dir w d)
  ...
  (cond
    [(and (opposite-dir? (snake-dir the-snake) d)
     ...
     (stop-with w)]
    [else (pit (snake-change-dir the-snake d) (pit-goos w) (pit-obstacles w))]))
