...
(provide lets-eat ;; String String[IP Address] -> Meal
...
(define (handle-server-messages meal msg)
  (cond
    [(app? meal) (handle-appetizer-message meal msg)]
...
(define (set-waypoint meal x y event)
  (if (and (entree? meal) (string=? event "button-down")) (make-package meal (list GOTO x y)) meal))

;; Meal -> Image
;; deals with draw some kind of meal
(define (render-the-meal meal)
  (cond
    [(app? meal) (render-appetizer meal)]
...
(define (handle-appetizer-message s msg)
  (cond
    [(id? msg) (app msg (app-img s) (app-countdown s))]
...
(define (score-list? l)
  (for/and ([s l])
    (and (list? s) (= SCORE-LIST-LENGTH (length s)) (id? (first s)) (number? (second s)))))
...
(define (render-id+image app)
  ...
  (overlay (cond
             ...
             [else
              (define s (string-append LOADING-OPEN-TEXT id))
...
(define (add-players id lof base-scene)
  (for/fold ([scn base-scene]) ([feaster lof])
    (place-image (render-avatar id feaster) (feaster-x feaster) (feaster-y feaster) scn)))
...
(define (render-avatar id player)
  ...
  (above (render-text (player-id player))
         (overlay (render-player-score player) PLAYER-IMG (circle size 'outline color))))
...
(define (add-path id players base-scene)
  ...
  (if (boolean? player) base-scene (add-waypoint* player base-scene)))
